...
export default function Edit({ indicator, themes }: EditProps) {
    const { data, setData, put, processing, errors } = useForm({
        code: indicator.code || '',
        name: indicator.name || '',
        description: indicator.description || '',
        theme_id: indicator.theme_id?.toString() || '',
        type: indicator.type || 'output',
        unit: indicator.unit || 'number',
        unit_label: indicator.unit_label || '',
        baseline_value: indicator.baseline_value?.toString() || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/indicators/${indicator.id}`);
    };

    return (
        <AdminLayout header="Edit Indicator">
