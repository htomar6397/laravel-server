...
class UpdateProjectRequest extends BaseRequest
{
    ...
    public function authorize()
    {
        ...
        // Users can edit their own projects or if they have edit permission
        return $this->user()->hasPermission('edit_projects') || 
               ($project && $project->created_by === $this->user()->id);
    }
    ...
    public function rules()
    {
        ...
        return [
            'code' => 'required|string|max:50|unique:projects,code,' . $projectId,
    ...
    protected function after($validator)
    {
        ...
        $project = $this->route('project');
        if ($project && $this->has('status')) {
            $newStatus = $this->input('status');
            $currentStatus = $project->status;

            // Prevent reactivating cancelled projects
            if ($currentStatus === 'CANCELLED' && in_array($newStatus, ['ACTIVE', 'PLANNING'])) {
                $validator->errors()->add('status', 'Cannot reactivate cancelled projects. Create a new project instead.');
            }
        }
    }
